...
class SegDiffEstimator(PyTorchLightningEstimator):
    """
    An estimator training the SegDiff model for forecasting as described in
    https://arxiv.org/abs/2211.14730 extended to be probabilistic.

    This class uses the model defined in ``SegDiffModel``,
    and wraps it into a ``SegDiffLightningModule`` for training
    purposes: training is performed using PyTorch Lightning's ``pl.Trainer``
    class.

    Parameters
    ----------
    context_length_multiplier
        The multiplier for the context length by patch length.
    patch_len
        Length of the patch.
    ...
    def __init__(
        ...
        context_length_multiplier: int = 10,
        d_model: int = 32,
        nhead: int = 4,
    ...
    def create_lightning_module(self) -> pl.LightningModule:
        return SegDiffLightningModule(
            lr=self.lr,
            weight_decay=self.weight_decay,
            model_kwargs={
                "patch_len": self.patch_len,
                "prediction_length": self.prediction_length,
                "context_length_multiplier": self.context_length_multiplier,
    ...
    def _create_instance_splitter(
        self, module: SegDiffLightningModule, mode: str
    ):
    ...
    def create_validation_data_loader(
        self, data: Dataset, module: SegDiffLightningModule, **kwargs
    ) -> Iterable:
