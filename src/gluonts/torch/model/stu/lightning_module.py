...
from .module import STUModel


class STULightningModule(L.LightningModule):
    """
    A ``pl.LightningModule`` class that can be used to train a ``STUModel``
    with PyTorch Lightning.

    This is a thin layer around a (wrapped) ``STUModel`` object,
    that exposes the methods to evaluate training and validation loss.

    Parameters
    ----------
    model_kwargs
        Keyword arguments to construct the ``STUModel`` to be trained.
    loss
    ...
    def __init__(
        ...
        self.model = STUModel(**model_kwargs)
        self.lr = lr
