...
from .module import MambaModel


class MambaLightningModule(pl.LightningModule):
    """
    A ``pl.LightningModule`` class that can be used to train a
    ``MambaModel`` with PyTorch Lightning.

    This is a thin layer around a (wrapped) ``MambaModel`` object,
    that exposes the methods to evaluate training and validation loss.

    Parameters
    ----------
    model_kwargs
        Keyword arguments to construct the ``MambaModel`` to be trained.
    loss
    ...
    def __init__(
        ...
        self.model = MambaModel(**model_kwargs)
        self.loss = loss
        self.lr = lr
