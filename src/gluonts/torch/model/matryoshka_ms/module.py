...
class MatryoshkaMSModel(nn.Module):
    """
    Module implementing the MatryoshkaMS model.
    ...
    num_prediction_steps
        Number of prediction steps per time step.
    dropout_rate
    ...
    def __init__(
        ...
        self.d_model = 2**(num_prediction_steps + 1)
        
        self.param_proj =  nn.ModuleList(
            [self.distr_output.get_args_proj(2**dim) for dim in range(1, num_prediction_steps + 1)]
        )
        ...
        self.transformer_input_size = len(self.lags_seq) + self._number_of_features

        self.projection = nn.Linear(self.transformer_input_size, self.d_model)
        decoder_layer = nn.TransformerEncoderLayer(d_model=self.d_model, nhead=num_prediction_steps)
    ...
    def _past_length(self) -> int:
        return self.context_length + max(self.lags_seq) + self.num_prediction_steps
    ...
    def prepare_transformer_input(
        ...
        context = past_target[..., -self.context_length - self.num_prediction_steps :]
        observed_context = past_observed_values[..., -self.context_length - self.num_prediction_steps :]
        ...
        prior_input = past_target[..., : -self.context_length - self.num_prediction_steps] / scale
        ...
        if future_target is not None:
            ...
            time_feat = torch.cat(
                (
                    take_last(past_time_feat, dim=-2, num=self.context_length  + self.num_prediction_steps - 1),
                    future_time_feat,
                ),
                dim=-2,
            )
        else:
            time_feat = take_last(past_time_feat, dim=-2, num=self.context_length  + self.num_prediction_steps)
    ...
    def transformer_decoder(
        ...
        """
        Applies the underlying Transformer to the provided target data and covariates.
        ...
        """
        transformer_input, scale, static_feat = self.prepare_transformer_input(
            feat_static_cat,
            feat_static_real,
            past_time_feat,
            past_target,
        ...
        output = self.projection(transformer_input)
        output = self.decoder(output)

        # parameter projection using larger and larger part of the output
        params = [
            param_proj(output[..., : 2**dim])
            for dim, param_proj in zip(range(1, self.num_prediction_steps + 1), self.param_proj)
        ]
    ...
    def forward(
        ...
        repeated_future_time_feat = future_time_feat[
            :, : self.prediction_length, :
        ]
        repeated_feat_static_cat = feat_static_cat.repeat_interleave(
            repeats=num_parallel_samples, dim=0
        )
        repeated_feat_static_real = feat_static_real.repeat_interleave(
            repeats=num_parallel_samples, dim=0
        )
        ...
        for k in range(self.prediction_length):
            ...
            all_params, scale, _, static_feat = self.transformer_decoder(
                repeated_feat_static_cat,
                repeated_feat_static_real,
                repeated_past_time_feat,
                repeated_past_target,
                repeated_past_observed_values,
                future_time_feat=repeated_future_time_feat[..., : k + 1, :],
            )
            sliced_params = [p[:, -1:] for p in all_params[0]]
            distr = self.distr_output.distribution(sliced_params, scale=scale)
            sample = distr.sample()
    ...
    def loss(
        ...
        params, scale, _, _ = self.transformer_decoder(
            feat_static_cat,
            feat_static_real,
            past_time_feat,
            past_target,
        ...
        # concate the params for all but the last num_prediction_steps  
        params = [(p[:, :-self.num_prediction_steps] for p in param) for param in params]

        loss_values = []
        for step, param in enumerate(params):
            context_target = take_last(
                past_target, dim=-1, num=self.context_length + self.num_prediction_steps - 1 - step
            )
            target = torch.cat(
                (context_target, future_target_reshaped[:, :-self.num_prediction_steps + step]),
                dim=1,
            )
            context_observed = take_last(
                past_observed_values, dim=-1, num=self.context_length + self.num_prediction_steps - 1 - step
            )
            observed_values = torch.cat(
                (context_observed, future_observed_reshaped[:, :-self.num_prediction_steps + step]), dim=1
            )

            loss = self.distr_output.loss(
                target=target, distr_args=param, scale=scale
            )
            loss = loss * observed_values

            loss = loss.reshape(*batch_shape, *loss.shape[1:])

            loss_values.append(aggregate_by(
                loss,
                dim=tuple(range(extra_dims + 1, len(future_target.shape))),
            ))

        return torch.stack(loss_values, dim=0).mean(dim=0)
